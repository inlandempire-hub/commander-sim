...
describe("Psychic Frog", () => {
  it("Discard a card: put a +1/+1 counter on it", () => {
    ...
    activateAbility(state, alice.id, frog.instanceId, 0, [], { discardInstanceIds: [fodder.instanceId] });
    drain(state);
  ...
  it("the discard cost is refused with no card to discard", () => {
    ...
    expect(() => activateAbility(state, alice.id, frog.instanceId, 0, [], { discardInstanceIds: [] })).toThrow(
      /discard/i,
    );
  });
