...
describe("Supremacy batch 16", () => {
  ...
  it("Ambitious Farmhand: transforms to Seasoned Cathar only under coven", () => {
    ...
    // No coven yet: activating throws.
    expect(() => activateAbility(state, alice.id, farmhand.instanceId, 0)).toThrow();
    // Three creatures of different powers -> coven on.
    const a = createCardInstance(state, "grizzly-bears", alice.id, "battlefield"); // 2
    const b = createCardInstance(state, "helpful-hunter", alice.id, "battlefield"); // 1
    // farmhand is 1, grizzly 2, hunter 1 -> powers {1,1,2} = 2 distinct. Add a 3-power.
    createCardInstance(state, "necropolis-regent", alice.id, "battlefield"); // 6
    void a; void b;
    alice.manaPool = { W: 3 };
  ...
  it("Null Elemental Blast: only multicolored permanents are legal targets", () => {
    ...
    const gold = createCardInstance(state, "glissa-sunslayer", bob.id, "battlefield"); // B/G multicolored
